...
export function Preview() {
  ...
  const generatePdf = async () => {
    try {
      ...
      let arabicPageCount = settings.pageNumbering.startFrom;
      let currentY = marginTop;
      let skipPageNumber = false;

      const addPageNumber = (isRoman: boolean) => {
        if (settings.pageNumbering.enabled && !skipPageNumber) {
      ...
      // Start frontmatter with Roman numerals
      ...
      if (settings.tableOfContents.enabled) {
        ...
        for (const chapter of [...mainChapters, ...backmatterChapters]) {
          ...
          doc.text(String(chapter.pageNumber || arabicPageCount), pageWidth - marginRight, currentY, { align: 'right' });
          currentY += settings.fonts.subtitle.lineHeight * settings.fonts.subtitle.size * 0.352778;
      ...
      for (const chapter of mainChapters) {
        ...
        // Chapter content pages - with page numbers
        doc.addPage();
        skipPageNumber = false;
        currentY = marginTop;
        addPageNumber(false);
      ...
      // Backmatter with Arabic numerals
      const backmatterChapters = chapters.filter(ch => ch.type === 'backmatter');
